...
an extant loaded kenrel module
...
The fact that this discussion is placed right under
the usage of the `module_init` and `module_exit` macros
with the desired `init` and `exit` functions passed
respectively as the sole parameter to each one
may have given away the final piece of the puzzle.
Indeed, this usage is exactly the method we use
to set the values of these function pointers
to the addresses of our desired `init` and `exit` functions.
