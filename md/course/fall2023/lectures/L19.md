...
The fact that we placed this discussion right under
the usage of the `module_init` and `module_exit` macro
is a blatant hint that
indeed, this usage is exactly the method we use
to set these members of the `struct module`
generated for us at build time.
